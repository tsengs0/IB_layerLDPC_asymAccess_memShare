...
void ch_ram_pass_test_pattern(unsigned int submatrix_id);
...
/*--------------------------------------------*/
void shiftFactor_paramFile_gen();
/*--------------------------------------------*/
...
/*----------------------------------------------------------------------------------------------------*/
	...
	for(unsigned int submatrix_id = 0; submatrix_id < dc; submatrix_id++) {
		v2c_msg_pass_test_pattern(submatrix_id);
		ch_ram_pass_test_pattern(submatrix_id);
	}
	// To generate the Test Pattern for HDL verification
	//for(unsigned int submatrix_id = 1; submatrix_id < dc; submatrix_id++)
...
/*------------------------------------------------------------------------------------------*/
// Test pattern type 1. extrinsic messages which are input to associated barrel shifter (BS)
/*------------------------------------------------------------------------------------------*/
	sprintf(filename, "submatrix_%d_chRAM_init.mem", submatrix_id);
	ofstream hdl_fd; hdl_fd.open(filename, ofstream::out);

	unsigned int chLayer_bs_out[layer_num][inLayer_cnt][Pc];
	...
	for(unsigned int layer_id = 0; layer_id < layer_num; layer_id++) {
		for(unsigned int row_chunk_id = 0; row_chunk_id < inLayer_cnt; row_chunk_id++) {
			for(unsigned int col = 0; col < Pc; col++) {
				if(layer_id == 0) {
					rand_temp = rand() % ((unsigned int) 1 << q); //col % ((unsigned int) 1 << q);
					...
					chLayer_bs_out[layer_id][norm_row1][norm_col1] = rand_temp;
				}
				else {
					norm_col = (submatrix_id*z)+(Pc*row_chunk_id)+col;

					// destination of current layer
					norm_row1 = layer[layer_id].PA[norm_col];
					norm_col1 = layer[layer_id].DS[norm_col];

					unsigned int prev_norm_row1 = layer[layer_id-1].PA[norm_col];
					unsigned int prev_norm_col1 = layer[layer_id-1].DS[norm_col];
					// destination of previous layer
					chLayer_bs_out[layer_id][norm_row1][norm_col1] = chLayer_bs_out[layer_id-1][row_chunk_id][col];	
				}
			}
			if(!(layer_id == layer_num-1 && row_chunk_id == inLayer_cnt-1))
				hdl_fd << endl;
		}		
	}
/*------------------------------------------------------------------------------------------*/
// Test pattern type 2. extrinsic messages which are output from associated barrel shifter (BS)
/*------------------------------------------------------------------------------------------*/
	sprintf(filename, "submatrix_%d_chLayer_bs_out.mem", submatrix_id);
	ofstream hdl_fd_bsOut; hdl_fd_bsOut.open(filename, ofstream::out);
	for(unsigned int layer_id = 0; layer_id < layer_num; layer_id++) {
		for(unsigned int row_chunk_id = 0; row_chunk_id < inLayer_cnt; row_chunk_id++) {
			for(unsigned int col = 0; col < Pc; col++) {
				norm_col = (submatrix_id*z)+(Pc*row_chunk_id)+col;
				...
				hdl_fd_bsOut << hex << chLayer_bs_out[layer_id][norm_row1][col] << ",";
			}
			if(!(layer_id == layer_num-1 && row_chunk_id == inLayer_cnt-1))
				hdl_fd_bsOut << endl;
		}
...
/*------------------------------------------------------------------------------------------*/
// Test pattern type 3. extrinsic messages which are output from associated page alignment
/*------------------------------------------------------------------------------------------*/
	sprintf(filename, "submatrix_%d_chLayer_pa_out.mem", submatrix_id);
	ofstream hdl_fd_paOut; hdl_fd_paOut.open(filename, ofstream::out);
	for(unsigned int layer_id = 0; layer_id < layer_num; layer_id++) {
		norm_col = (submatrix_id*z)+(Pc*0)+(Pc-1);
		norm_row1 = layer[layer_id].PA[norm_col];
		if(submatrix_id==1) cout << "norm_row1: " << norm_row1 << endl;
		unsigned int start_page; start_page = norm_row1;
		for(unsigned int mem_page_wr_cnt = 0; mem_page_wr_cnt < inLayer_cnt; mem_page_wr_cnt++) {
			for(unsigned int col = 0; col < Pc; col++) {
				hdl_fd_paOut << hex << chLayer_bs_out[layer_id][start_page][col] << ",";
			}
			if(submatrix_id==1) cout << "sub_1_start_wr_page: " << start_page << endl;
			start_page = (start_page+1) % inLayer_cnt;
			if(!(layer_id == layer_num-1 && mem_page_wr_cnt == inLayer_cnt-1))
				hdl_fd_paOut << endl;
		}
	}
/*------------------------------------------------------------------------------------------*/
// Test pattern type 4. extrinsic messages which are output from associated MEMs
/*------------------------------------------------------------------------------------------*/
	sprintf(filename, "submatrix_%d_chLayer_mem_out.mem", submatrix_id);
	ofstream hdl_fd_memOut; hdl_fd_memOut.open(filename, ofstream::out);
	unsigned int drifted_layer_id;
	for(unsigned int layer_id = 0; layer_id < layer_num; layer_id++) {
		drifted_layer_id = (layer_id+(layer_num-1)) % layer_num; // 0 1 2 -> 2 0 1, i.e., (0+(3-1))%3=2, (1+2)%3=0, (2+2)%3=1
		for(unsigned int row_chunk_id = 0; row_chunk_id < inLayer_cnt; row_chunk_id++) {
			for(unsigned int col = 0; col < Pc; col++) {
				hdl_fd_memOut << hex << chLayer_bs_out[drifted_layer_id][row_chunk_id][col] << ",";
			}
			if(!(layer_id == layer_num-1 && row_chunk_id == inLayer_cnt-1))
				hdl_fd_memOut << endl;
		}
	...
	for(unsigned int layer_id = 0; layer_id < layer_num; layer_id++) {
		for(unsigned int row_chunk_id = 0; row_chunk_id < inLayer_cnt; row_chunk_id++) {
			for(unsigned int col = 0; col < Pc; col++) {
				...
				cout << "Col_" << submatrix_id << ", v2c_bs_out["
					 << dec << layer_id << "][" 
					 << dec << norm_row1 
					 << "][" 
					 << dec << norm_col1 
...
// Test pattern type 2. extrinsic messages which are output from associated barrel shifter (BS)
/*------------------------------------------------------------------------------------------*/
	sprintf(filename, "submatrix_%d_v2c_bs_out.mem", submatrix_id);
	ofstream hdl_fd_bsOut; hdl_fd_bsOut.open(filename, ofstream::out);
	for(unsigned int layer_id = 0; layer_id < layer_num; layer_id++) {
		for(unsigned int row_chunk_id = 0; row_chunk_id < inLayer_cnt; row_chunk_id++) {
			for(unsigned int col = 0; col < Pc; col++) {
				...
				hdl_fd_bsOut << hex << v2c_bs_out[layer_id][norm_row1][col] << ",";
			}
			if(!(layer_id == layer_num-1 && row_chunk_id == inLayer_cnt-1))
				hdl_fd_bsOut << endl;
		}
...
// Test pattern type 3. extrinsic messages which are output from associated page alignment
/*------------------------------------------------------------------------------------------*/
	sprintf(filename, "submatrix_%d_v2c_pa_out.mem", submatrix_id);
	ofstream hdl_fd_paOut; hdl_fd_paOut.open(filename, ofstream::out);
	for(unsigned int layer_id = 0; layer_id < layer_num; layer_id++) {
		norm_col = (submatrix_id*z)+(Pc*0)+(Pc-1);
		norm_row1 = layer[layer_id].PA[norm_col];
		...
		for(unsigned int mem_page_wr_cnt = 0; mem_page_wr_cnt < inLayer_cnt; mem_page_wr_cnt++) {
			for(unsigned int col = 0; col < Pc; col++) {
				hdl_fd_paOut << hex << v2c_bs_out[layer_id][start_page][col] << ",";
			}
			if(submatrix_id==1) cout << "sub_1_start_wr_page: " << start_page << endl;
			start_page = (start_page+1) % inLayer_cnt;
			if(!(layer_id == layer_num-1 && mem_page_wr_cnt == inLayer_cnt-1))
...
// Test pattern type 4. extrinsic messages which are output from associated MEMs
/*------------------------------------------------------------------------------------------*/
	sprintf(filename, "submatrix_%d_v2c_mem_out.mem", submatrix_id);
	ofstream hdl_fd_memOut; hdl_fd_memOut.open(filename, ofstream::out);
	for(unsigned int layer_id = 0; layer_id < layer_num; layer_id++) {
		for(unsigned int row_chunk_id = 0; row_chunk_id < inLayer_cnt; row_chunk_id++) {
			for(unsigned int col = 0; col < Pc; col++) {
				hdl_fd_memOut << hex << v2c_bs_out[layer_id][row_chunk_id][col] << ",";
			}
			if(!(layer_id == layer_num-1 && row_chunk_id == inLayer_cnt-1))
				hdl_fd_memOut << endl;
		}
...
void shiftFactor_paramFile_gen()
{
	unsigned int shift_temp;
	char *factor_filename="routeNetwork_shiftFactor_param.vh";
	ofstream factor_fd; factor_fd.open(factor_filename, ofstream::out);
	for(int sub_matrix_id = 0; sub_matrix_id < dc; sub_matrix_id++) {
		factor_fd << "//==========================================================================//" << endl
			      << "// Starting page address of base-matrix colmumn " << sub_matrix_id << " and BS shift control" << endl;
		for(int layer_id=0; layer_id<layer_num; layer_id++) {
			#ifdef DEBUG
				std::cout << "Layer_" << layer_id << ", Submatrix_" << sub_matrix_id << ": " 
				          << dec << layer[layer_id].shift_factor[sub_matrix_id] << std::endl;
			#endif

			// zero shifting or others
			shift_temp = (layer[layer_id].shift_factor[sub_matrix_id]==Pc) ? 0 : layer[layer_id].shift_factor[sub_matrix_id];
			factor_fd << "`define shift_factor_" << sub_matrix_id << "_" << layer_id << " `ROW_PARALLELISM-" 
			          << dec << shift_temp << endl;
		}
	}
